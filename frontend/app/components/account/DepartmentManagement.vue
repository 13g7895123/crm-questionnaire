  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UButton
          ...
          color="primary"
          variant="solid"
          :label="$t('departments.createDepartment')"
      ...
      <UCard>
        <template #header>
          <h3 class="text-lg font-semibold">
            {{ isEditing ? $t('departments.editDepartment') : $t('departments.createDepartment') }}
          </h3>
        </template>

        <form @submit.prevent="handleSubmit" class="space-y-4">
          <UFormGroup :label="$t('departments.departmentName')" required>
    ...
    <UModal v-model="isDeleteModalOpen">
      <UCard>
        ...
        <p>{{ $t('departments.confirmDelete') }}</p>
        <template #footer>
